            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $role->name) }}" placeholder="Contoh: Administrator HR"
                        class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-ink focus:border-navy focus:ring-1 focus:ring-navy outline-none transition {{ $role->exists && $role->is_system ? 'bg-slate-100 text-slate-500 cursor-not-allowed' : '' }}" {{ $role->exists && $role->is_system ? 'readonly' : '' }} required>
                </div>
                <div>
                    ...
                    <input type="text" name="slug" value="{{ old('slug', $role->slug) }}" placeholder="Contoh: admin-hr"
                        class="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-mono text-ink focus:border-navy focus:ring-1 focus:ring-navy outline-none transition {{ $role->exists && $role->is_system ? 'bg-slate-100 text-slate-500 cursor-not-allowed' : '' }}" {{ $role->exists && $role->is_system ? 'readonly' : '' }} required>
            ...
            @if($role->exists && $role->is_system)
                <div class="p-4 bg-amber-50 border border-amber-200 rounded-xl">
                    <p class="text-sm font-medium text-amber-800 flex items-center gap-2">
                        ...
                        Role sistem: nama &amp; slug terkunci — permission tetap dapat diubah.
                    </p>
                </div>
            @endif
